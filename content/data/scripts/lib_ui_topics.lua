...
local topics = {
	...
	loadscreen = {
		...
		load_bar = Topic('LoadingBar'), --Expects an image file returned, replaces the loading screen bar. Should be an animation. Sends the UI context.
		bg_class = Topic(nil), --Expects a CSS class name returned (or nil). Replaces the mission-filename background class on the load screen. Sends the UI context.
		tip_text = Topic(nil), --Expects a tip string returned (or nil) to populate the #loadscreen_tip element. All tip styling is done via RCSS. Sends the UI context.
		unload = Topic(nil) --Runs arbitrary script and expects no return value. Sends the document context
	},
	briefcommon = {
		initialize = Topic(nil), --Runs arbitrary script and expects no return value. Sends the document context
	...
	loadouts = {
		...
		beforeFillShipSlot = Topic(true), --Runs arbitrary script and expects a boolean return for if the ship slot fill should continue. Sends the ship slot and the ship class index
		beforeFillWeaponSlot = Topic(true), --Runs arbitrary script and expects a boolean return for if the weapon slot fill should continue. Sends the ship slot, the weapon bank, and the weapon class index
		...
		emptyWeaponSlot = Topic(nil), --Runs arbitrary script and expects no return value. Sends the ship slot and the weapon bank
		fillWeaponSlot = Topic(true), -- Runs arbitrary script and expects a boolean return for if the weapon slot fill was successful. Sends the ship slot and the weapon bank
